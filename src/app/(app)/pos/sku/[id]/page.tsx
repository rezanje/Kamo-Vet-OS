...
export default async function BarangDetailPage({
  ...
  const { supabase, categories, brands, units, suppliers, barangLain } = await siapkanFormBarang();
  ...
  return (
    <>
      ...
      <BarangForm categories={categories} brands={brands} satuanMaster={units} suppliers={suppliers} barangLain={barangLain} editing={barang} />
    </>
  );
}
