...
import { assertNewAssetFunding, assetPurchaseJournal } from "@/lib/asset-acquisition";
...
async function bacaAset(supabase: Awaited<ReturnType<typeof createClient>>, formData: FormData): Promise<AssetInput> {
  ...
  if (!nama || !categoryId || harga <= 0 || umurBulan <= 0) {
    redirect(`${back}?error=${encodeURIComponent("Nama, kategori, harga perolehan, dan umur ekonomis wajib diisi")}`);
  }
  if (nilaiSisa < 0 || nilaiSisa >= harga) redirect(`${back}?error=${encodeURIComponent("Nilai sisa harus lebih kecil dari harga perolehan")}`);
  const { data: kat } = await supabase.from("asset_categories").select("nama").eq("id", categoryId).eq("is_active", true).maybeSingle();
...
export async function tambahSaldoAwalAset(formData: FormData) {
  ...
  const { error } = await supabase.from("fixed_assets").insert({
    ...
    umur_bulan: a.umurBulan, branch_id: a.branchId, acquisition_kind: "opening_balance",
  });
...
export async function tambahPembelianAset(formData: FormData) {
  ...
  try { assertNewAssetFunding(sumber); } catch (e) {
  ...
  const creditCode = sumber === "accounts_payable"
    ? "2101"
    : await kodeAkunBayar(supabase, sumber === "cash" ? "Tunai" : "Transfer", a.branchId, String(formData.get("account_id") ?? "").trim() || null);
  try { assetPurchaseJournal(a.harga, sumber, creditCode); } catch (e) {
    redirect(`${back}?error=${encodeURIComponent(e instanceof Error ? e.message : "Sumber dana tidak valid")}`);
  }
  const { error } = await supabase.rpc("create_fixed_asset_purchase", {
    p_nama: a.nama, p_category_id: a.categoryId, p_tanggal: a.tanggal,
    p_harga: a.harga, p_nilai_sisa: a.nilaiSisa, p_umur_bulan: a.umurBulan,
    p_branch_id: a.branchId, p_credit_code: creditCode,
  });
  if (error) redirect(`${back}?error=${encodeURIComponent(`Aset dan jurnal dibatalkan: ${error.message}`)}`);
