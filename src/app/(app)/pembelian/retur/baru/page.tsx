...
type PoRow = {
  ...
  purchase_order_items: { item_id: string | null; qty: number; qty_terima: number | null; harga_beli: number; nama: string; faktor: number | null }[] | null;
};
...
export default async function ReturBeliBaruPage({
  ...
  const [{ data: pos }, { data: returns }] = await Promise.all([
    supabase
      ...
      .select("id, no_po, tanggal, suppliers(nama), purchase_order_items(item_id, qty, qty_terima, harga_beli, nama, faktor)")
      .eq("status", "Diterima")
  ...
  const options: PoOption[] = ((pos ?? []) as unknown as PoRow[]).map((p) => {
    ...
    // hanya barang yang benar-benar diterima yang bisa diretur
    for (const r of p.purchase_order_items ?? []) {
      if (!r.item_id) continue;
      // satuan dasar — sama dgn actions.ts, karena stok yg dipotong juga satuan dasar
      const f = Number(r.faktor) > 0 ? Number(r.faktor) : 1;
      sumber[r.item_id] = (sumber[r.item_id] ?? 0) + toBaseQty(qtyDiterima(r), f);
      meta[r.item_id] = { nama: r.nama, harga: toBaseCost(Number(r.harga_beli) || 0, f) };
    }
